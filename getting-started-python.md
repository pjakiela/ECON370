...
Download the Python distribution anaconda from the [anaconda](https://www.anaconda.com/download/success) website using the distribution installers links on the left. Once you've done this, 
you should be able to open spyder on your computer. You should see three panels within spyder. The one on the left is the script editor where you will write your python scripts, and the one on 
the bottom right is the IPython console, where you can enter commands interactively. For example, if you type 
```
x = 2 + 5
```
in the IPython console and hit enter, you will see `x` appear in the Variable Explorer pane in the upper right corner of the screen. You have defined `x` as 7 (the sum of 2 and 5). Now, if you 
...
The python code below loads the tidyverse and haven libraries and loads a Stata data set directly from the web. 
```
...
Copy this example into a new python program on your computer and run the code. Once you run the code, you should see the data frame `bl` listed in the `Variable Explorer` tab 
If you enter `bl` in the console pane in the lower left part of your screen, R will print the first few rows of the data frame `bl`.  
