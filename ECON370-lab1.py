...
data_path = f"{mypath}\\data\\BL2013_MF1599_v2.2.csv"
bldata = pd.read_csv(data_path)

## how many observations are in the data set?  how many variables?  
## which variables are string (or character) variables?

## R suggestions:  spec(), head(), summary(), dim()
## Python suggestions: type(), df.info, df.shape, df.columns, df.dtypes



## define a new dataframe called bl2010 which only contains BL data from 2010
...
## make a table summarizing the average rate of educational attainment by region

## R suggestion:  use group_by() and pipe to summarize()
## Python suggestion:  use df.groupby().agg()

summary = bl2010.groupby('wb_region').agg(
    edu=('mean_edu', 'mean'),
    n=('mean_edu', 'size')
)
...
bl2010['high_income'] = np.where(bl2010['wb_region'].str.contains('Advanced'), 1, 0)
