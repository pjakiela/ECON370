...
pjpath <- paste0("C:/Users/", username, "/Dropbox/ECON-370/topics/11-clusters/lab/")
...
urlfile <- 'https://raw.githubusercontent.com/pjakiela/ECON370/refs/heads/gh-pages/ECON370-lab11-data.csv'
nber <- read_csv(urlfile)
...
nber <- nber %>% 
  filter(Development_Economics == 1 | Children_and_Families == 1) %>% 
  select(wp_number, title, abstract, Development_Economics, Children_and_Families)
...
unique_words <- nber_tf %>% 
  mutate(dummy = ceiling(proportion)) %>% 
  group_by(word) %>% 
  summarize(count = n()) %>% 
  filter(count <= 1)

nber_tf_final <-  nber_tf %>% 
  anti_join(unique_words)

## create a document term matrix

nber_dtm <- nber_tf_final %>% 
...
look_at_clusters <- look_at_clusters %>% 
  ...
  summarize(n = n(), 
            ...
            mean_childrens = mean(Children_and_Families))
look_at_clusters
...
##    rerun it a few times after commenting out the set.seed line
