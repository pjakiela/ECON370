...
var_importance <- tibble(varname = colnames(X), importance = cf_importance[,1]) %>% 
  arrange(desc(importance))
print(var_importance, n=10)

## there is no hard and fast rule for choosing important predictors...
cf_select_vars <- var_importance %>% 
  filter(importance >= mean(cf_importance))
cf_select_vars
